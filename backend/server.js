...
for (const p of persisted) {
    sessions.set(p.id, {
        ...
        serializeAddon: null,
        promptSalt: null,
        // SSH Path B state. See top of attachPtyHandlers + startCommand for
...
function createSession(sessionId) {
  ...
  const session = {
    ...
    serializeAddon: null,
    promptSalt: null,
    // SSH Path B state — see attachPtyHandlers / startCommand. Runtime-only.
    sshActive: false,
    sshHost: null,
    sshOuterCellId: null,
    sshPromptSalt: null,
...
// out), we treat it as a Path B candidate. The high-level lifecycle:
//
...
function attachPtyHandlers(session) {
  ...
  ptyProcess.onData((dataStr) => {
      ...
      // SSH Path B: in 'pending' state, watch tailBuf for a remote prompt.
      ...
      // SSH-aware salt list: when we've injected a Path B integration into a
      ...
      // Path B" still works for users on shells with built-in OSC 133
      ...
      // marker can close a Path B cell).
      ...
      if (session.activeCellId) {
        ...
        if (finishMatch && !session.isTuiActive) {
            // ── SSH Path B: special handling for the BOOTSTRAP success signal ──
            ...
            // ── SSH Path B: if a which='local' finishMatch arrived while
...
function startCommand(session, cellId, commandData) {
    ...
    if (session.tailBuf.length > 0 && !session.tailBuf.includes('\x1b[2J')) {
        // In an active Path B SSH cell, tailBuf at this point contains the
