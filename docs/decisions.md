...
the the SSH integration feature landed). The `allowUnsalted` opt-in flag controls
...
**Nested ssh**: only the OUTERMOST ssh gets SSH integration. An inner
...
asserts unsalted markers from remote do NOT close the SSH integration cells).
...
### Why on by default (not the pre-integration SSH mode) {#ssh-path-b-rationale}

**Was considered**: the pre-integration SSH mode (one SSH = one big cell with full passthrough,
no remote cells). Safe, simple, predictable. The earlier "pre-integration"
...
**Why the SSH integration won**: each remote command being a proper cell with real
...
fails, we degrade to the pre-integration SSH mode automatically — no worse than where we
started.
...
**Decision**: When in an the active SSH integration SSH session, the chat input's Tab
...
### Control-key forwarding from chat input with the SSH integration active {#ssh-ctrl-forwarding}
...
**Decision**: when in an the active SSH integration SSH session, show the remote
...
**Decision**: Ctrl+D on empty chat input during the SSH integration SSH does NOT
