...
6. Add `shot(page, testInfo, 'label')` at key states for inspection.
...
wrapper that requires hardware key taps on each connection, set
`TERMBOOK_E2E_SSH_BIN=/usr/bin/ssh` (already the default in
