...
function App() {
  ...
  // Per-session the SSH integration activation state. When true, the chat input
    ...
    const connectWebSocket = () => {
        ...
        ws.onmessage = (event) => {
            ...
            if (msg.type === 'clear_history') {
                ...
                // the SSH integration state on reconnect / fresh join.
  ...
  const handleCommand = (e) => {
    ...
    // ── the SSH integration: control-key forwarding when idle in remote session ──
    // In a real terminal, Ctrl+D at an empty prompt EOFs the shell. In
    // the SSH integration between remote commands, the chat input is idle but the SSH
      ...
      <div className="main-area">
        <div className="top-header">
           ...
           {/* SSH session indicator: always-visible chip next to pwd, so the
               ...
               when the active session is in an the active SSH integration SSH session. */}
          ...
          <div className={`chat-input-wrapper${isPassthrough ? ' is-passthrough' : ''}${activeTuiState ? ' is-tui' : ''}${sessionSshActive[activeSessionId] ? ' is-ssh' : ''}`}>
            <span className="pwd-prompt-prefix">
              {isPassthrough ? (
                ...
                // In the SSH integration, replace the generic "termbook" prefix with a
