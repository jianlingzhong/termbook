...
test.describe('Session Management & Persistent State', () => {
  ...
  test('Creates a default session and tracks PWD accurately', async ({ page }) => {
    ...
    await expect(page.locator('.notebook-cell')).toHaveCount(1, { timeout: 15000 });
    ...
    await expect(page.locator('.notebook-cell')).toHaveCount(2, { timeout: 15000 });
  });
  ...
  test('Multiple sessions isolate state correctly without cross-pollution', async ({ page }) => {
    ...
    await expect(page.locator('.notebook-cell')).toHaveCount(0, { timeout: 5000 });
    ...
    await expect(page.locator('.notebook-cell')).toHaveCount(1, { timeout: 5000 });
    ...
    await expect(page.locator('.notebook-cell')).toHaveCount(2, { timeout: 15000 });
    const outputA2 = page.locator('.notebook-cell').nth(1).locator('.cell-output');
