...
**Scope**: feature-complete personal-tool web terminal. Active
feature development paused after this release; bug fixes welcome.
...
- **SSH "Path B" integration**: typing `ssh user@host` auto-injects
  a salted shell integration so each REMOTE command becomes its
  own cell with real remote pwd / git / exit code. Tab completion
  routes to the remote shell. The host appears next to the input
  prompt so you can't accidentally type thinking you're local.
  `--no-termbook` opts out. Absolute paths like `/usr/bin/ssh` are
  recognized too (helpful when the system `ssh` is shadowed).
...
- The WebGL renderer silently falls back to DOM if GPU is
  ...
  navigation.
- SSH "Path B" salted integration is plaintext in the remote
  shell's environment — same threat model as the local salt.

### Out of scope (no plans to add)

- **AI integration.** The structured cell data would be perfect for
  an LLM, but isn't built. Termbook leaves you to use Warp or paste
  cells into your tool of choice.
- **Multi-user / sharing / collaboration.** Single-user only.
- **Native packaging** (Electron, etc.). It's a browser app.
- **Settings UI.** Configuration is in `app_config.json`.
